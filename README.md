...
llm_connection:
  model: "Qwen3-32B"                       # 【必改】模型名，需与服务端一致
  server_url: "http://192.168.1.10:8000"   # 【必改】推理服务地址（见下方网络说明）
...
>
> - 推理服务在**别的机器**上：直接填那台机器的 IP，如 `http://192.168.1.10:8000`。
> - 推理服务在**本机（宿主机）**上：容器内的 `127.0.0.1` 指的是容器自己，不是宿主机。请用下方运行命令里的 `--add-host=host.docker.internal:host-gateway`，然后 `server_url` 填 `http://host.docker.internal:8000`。
...
| `--add-host=host.docker.internal:host-gateway` | 让容器能用 `host.docker.internal` 访问宿主机               |
